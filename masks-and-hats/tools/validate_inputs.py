        ...
        for path in paths:
            for f in os.listdir(path):
                if os.path.isdir(os.path.join(path,f)):
                    ...
                    if ext in img_extensions:
                        input_paths.append( os.path.join(path, f) )
    ...
    if potential_ext in img_extensions:
        ...
        if is_video == False:
            for p in input_paths:
                path = p.replace(p.split("/")[-1],'')
                path = path.replace(in_arg,out_arg)
                if not os.path.exists(path):
                    os.makedirs(path)
                f = p.split("/")[-1]
                f_name = ".".join( f.split(".")[:-1])
                f_ext = "." + f.split(".")[-1]
                f_out = f_name + f"_{args.suffix}" + f_ext
                out_p = os.path.join(path, f_out)
