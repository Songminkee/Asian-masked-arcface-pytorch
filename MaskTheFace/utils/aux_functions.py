...
patterns = []
folders = glob.glob('./masks/textures/*')
...
def mask_face(image, face_location, six_points, angle, args, type="surgical"):
    ...
    # if args.pattern and np.random.random()>0.5:
    #     # Apply pattern to mask
    #     global patterns
    if args.pattern:
        #pt = np.random.choice(patterns,1)[0]
        #img = texture_the_mask(img, pt, args.pattern_weight)
